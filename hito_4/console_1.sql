...
select concat('DBA II',' ','2022');
SELECT trim('        DBA II      ') AS TRIM;
SELECT '     DBA II' AS TRIM1;;
#CONCATENAR EL SIGUIENTE FORMATO DE SALIDA
#CONCATENAR NOMBRE Y APELLIDOS DE LA SIGUIENTE FORMA


create or replace function output_fullname(nombres varchar(20), apellidos varchar(20))
returns varchar (100)
begin
...
from estudiantes as est;
#
create or replace function output_age(gestion  varchar(20), edad varchar(20))
returns varchar (100)
begin
    declare resultado varchar (100) default '';
...
create or replace function output_age_and_fullname
    (nombres varchar(20), apellidos varchar(20),gestion varchar(20),edad varchar(29))
returns varchar (100)
begin
    declare resultado varchar (100) default '';
...
select output_age_and_fullname(est.nombres,est.apellidos,est.gestion,est.edad)as fullname_and_age
from estudiantes as est;

#############
#28
#a incritos en la gestion 2015
#b en la materia arq-11
#
select est.nombres,est.apellidos,i.semestre
    from estudiantes as est
inner join inscripcion i on est.id_est = i.id_est
    inner join materias m on i.id_mat = m.id_mat
where i.gestion=2016 and m.cod_mat='ARQ-101' AND i.semestre = '5to semestre';
####
select est.nombres,est.apellidos,i.semestre,i.gestion
    from estudiantes as est
inner join inscripcion i on est.id_est = i.id_est
